...
const Notifications = () => {
    ...
    return (
        <section
            className="lg:ml-64 min-h-screen bg-linear-to-r from-indigo-900  via-purple-900 to-pink-900
         text-white py-24 px-6 sm:px-10"
        >
            <div className=" max-w-5xl mx-auto">
                <div className="flex justify-between items-center mb-10">
                    <h2 className="text-4xl font-extrabold flex items-center gap-3">
                        <Bell className="w-8 h-8 text-cyan-400" /> إشعارات
                        الطلبات
                    </h2>
                ...
                {loading ? (
                    ...
                    <div className="space-y-4">
                        {notifications.map((n) => (
                            <div
                                key={n._id}
                                className={`bg-white/10 border border-white/20 backdrop-blur-md rounded-2xl p-5 flex justify-between items-center transition-all ${
                                    n.isRead
                                        ? "opacity-60"
